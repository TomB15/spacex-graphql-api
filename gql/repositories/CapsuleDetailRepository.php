...
/**
 * Class CapsuleDetailRepository
 * @package gql\repositories
 */
class CapsuleDetailRepository implements DataRepositoryInterface
{
    /**
    ...
    private function retrieveAllData()
    {
        ...
        $data = array_merge($data,
            $this->fetchData(Config::CAPSULE_DETAIL_DATA_URI['all']));
    ...
    private function retrieveFilteredData($arguments)
    {
        ...
        foreach ((array)$arguments['specific'] as $k => $argument){
            $data[] = array_merge($data,
                $this->fetchData(Config::CAPSULE_DETAIL_DATA_URI['all'] . '/' . $argument));
        }
